...
export interface MailAccount {
  id: string
  type: 'zoho' | 'gmail' | 'outlook' | 'smtp_imap' | 'other'
  email: string
  displayName: string
  ...
  warmupReplyDailyLimit: number
  warmupRepliesToday: number
  warmupOpensToday?: number
  warmupSpamRescuesToday?: number
  isActive: boolean
  ...
  warmupTimezone?: string | null
  warmupBusinessHoursStart?: string | null
  warmupBusinessHoursEnd?: string | null
  ...
  mailboxConnectionMethod?: 'imap' | 'api' | 'oauth' | 'graph' | 'unknown'
  microsoftConnectionStatus?: 'connected' | 'reauth_required' | 'legacy_requires_reconnect' | 'error' | null
  microsoftTokenExpiry?: string | null
  microsoftLastTokenRefreshAt?: string | null
  ...
  trulyInboxConnected?: boolean
  trulyInboxVisibleToOrg?: boolean
  trulyInboxHasApiKey?: boolean
  trulyInboxEmailAccountId?: number | null
  trulyInboxConnectedAt?: string | null
  trulyInboxLastSyncedAt?: string | null
  trulyInboxStatus?: string | null
  trulyInboxLastError?: string | null
...
export interface MailboxMessage {
  ...
  createdAt: string
  mailAccount: { email: string; displayName: string; type: 'zoho' | 'gmail' | 'outlook' | 'smtp_imap' | 'other' }
}
...
export interface DomainDiagnostics {
  domain: string
  providerHint: 'gmail' | 'zoho' | 'outlook' | 'unknown'
  checkedAt: string
  sharedProviderDomain: boolean
  mxHosts: string[]
  mxIps: string[]
  spf: {
    found: boolean
...
export type ActiveTab = 'accounts' | 'add-zoho' | 'add-gmail' | 'add-outlook' | 'add-smtp-imap'
